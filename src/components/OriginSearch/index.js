...
export default class HInputSearch extends React.Component {
  constructor(props) {
    super(props);
    const initialType = typeof props.value;
    this.state = {
      isShowSearch: false,
      loading: false,
      options: [],
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (isEmpty(value) && value !== this.props.value) {
      const initialType = typeof nextProps.value;
      this.setState({ value: nextProps.value, initialType });
    }
  }
  ...
  handleRemove() {
    ...
    const res = initialType === 'object' ? {} : undefined;
